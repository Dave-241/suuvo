...
const Footer = () => {
  ...
  return (
    <div
      className={` overflow-clip pt-[5rem]  bg-[#FFF8F6] flex flex-col relative w-full ${Sf_pro_medium.className}`}
    >
      {/* Modal Container */}
      <div className="w-full mx-auto md:w-[120rem] max-w-full  items-center gap-[0.15rem] flex flex-col z-[10]">
        <div className="relative w-full  items-center z-20 max-w-[97%]   grid grid-cols-1 md:grid-cols-2 gap-8 bg-[#EFEFEF]/81 backdrop-blur-[62px] rounded-[2rem] p-8  md:p-[3rem]">
          {/* Left Section */}
          <div
            className={`text-black md:pl-[5%] flex flex-col gap-[2rem] ${Sf_pro_bold.className}`}
          >
            <div className=" flex flex-col gap-[0.5rem] ">
              <h2 className="text-4xl md:text-[60px] font-bold leading-[105%]">
            ...
            <ul className=" flex flex-col gap-[1.2rem] text-sm md:text-base text-black">
              {benefits.map((e, index) => {
                return (
                  <li
                    key={index}
                    className={` text-lg flex items-start gap-4 ${Sf_pro_medium.className}`}
                  >
            ...
            <div
              className={`mt-4 flex items-center gap-4  ${Sf_pro_medium.className}`}
            >
              <button
          ...
          {/* Right Section: Form */}
          <form className="bg-white text-center rounded-[30px]  px-10 py-10 flex flex-col gap-4 text-sm md:text-base">
            <div className="flex flex-col gap-[0.4rem]">
              <h3
                className={`text-3xl font-bold text-black ${Sf_pro_bold.className}`}
              >
                Pre-Register Now
              </h3>
            ...
            <div className="flex flex-col gap-[1rem]">
              ...
              <textarea
                ...
                className="bg-[#000000]/5 rounded-[12px] h-[3.5rem] md:h-[7rem] p-5 border-black/0 border focus:border-black/30 transition duration-300 outline-none"
              />

              <div className="flex justify-center w-full ">
                <button
                  className={` bg-[#F3A21B] w-fit  capitalize ${Sf_pro_bold.className} rounded-full md:px-[5rem] text-lg mt-2 py-[1rem] text-[#3C1702]`}
                  style={{
      ...
      <div className=" absolute  w-[120rem] max-w-full left-[50%] translate-x-[-50%] bottom-0">
        <Image
          src={footerImg}
          alt="footer image"
          className="w-full h-fit  object-cover"
